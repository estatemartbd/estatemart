...
let updatePropertyClickCount = () => {
    return `UPDATE ${table_name} SET total_device_click_count = total_device_click_count + 1 WHERE id = ?`;
}

let getFeaturedListByDate = () => {

    let query = `SELECT *FROM ${table_name} WHERE  status = 1 and is_featured = 1 and (? BETWEEN feature_start_date AND feature_end_date)
    ORDER BY feature_start_date DESC  LIMIT ? OFFSET ?`;
