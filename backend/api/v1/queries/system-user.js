...
let table_name = "esm_system_users";
let user_table_name = "esm_users";


let getAllSystemUserList = () => {

    return `SELECT ${table_name}.id , ${table_name}.name , ${user_table_name}.email ,
    ${user_table_name}.phone ,  ${table_name}.status , ${table_name}.user_type, ${table_name}.organization_name,${table_name}.organization_details,
    ${user_table_name}.id as user_id, ${user_table_name}.role_id as role_id  FROM ${table_name} 
    JOIN ${user_table_name} on (${table_name}.id = ${user_table_name}.profile_id) 
    where ${user_table_name}.role_id != 1
    order by ${table_name}.id DESC`;
};

let getListByStatus = (status = 0) => {
    return `SELECT ${table_name}.id , ${table_name}.name , ${user_table_name}.email ,
    ${user_table_name}.phone ,  ${table_name}.status , ${table_name}.organization_name,${table_name}.organization_details,
    ${user_table_name}.id as user_id, ${user_table_name}.role_id as role_id FROM ${table_name} 
    JOIN ${user_table_name} on (${table_name}.id = ${user_table_name}.profile_id) 
    where ${user_table_name}.role_id != 1 and ${table_name}.status = ${status}
    order by ${table_name}.id DESC`;
};

let getSystemUserById = () => {
    return `SELECT * FROM ${table_name} where  id = ?  and status = 1 `;
};

let getSystemUserDetailsById = () => {
    return `SELECT * FROM ${table_name} where  id = ?   `;
};

let addNewSystemUser = () => {
    return `INSERT INTO ${table_name} SET ?`;
};

let disableSystemUserById = () => {
    return `UPDATE ${table_name} set status = 2 , updated_by= ? , updated_at = ? where id = ?  `;
};

let enableSystemUserById = () => {
    return `UPDATE ${table_name} set status = 1 , updated_by= ? , updated_at = ? where id = ?  `;
};

let updateSystemUserProfileById = (data) => {
