...
let table_name = "esm_admins";
let user_table_name = "esm_users";

let getAllAdminList = () => {
  return `SELECT ${table_name}.id , ${table_name}.name ,  ${table_name}.email ,
...
let getAdminById = () => {
  return `SELECT id, name, email, phone, profile_image, address, status FROM ${table_name} where  id = ?  and status = 1 `;
};
