...
let table_name = "esm_users";


let getUserByUserName = () => {
    return `SELECT id, email,phone, status,role_id,profile_id FROM ${table_name} where  user_name = ?`;
}

let getUserByUserNameOrEmail = () => {
    return `SELECT id, email,phone, status,role_id,profile_id FROM ${table_name} where  user_name = ? or email = ?`;
}

let getUserById = () => {
    return `SELECT id, email,phone, status,role_id,profile_id,password FROM ${table_name} where  id = ?  and status = 1 `;
}


let getUserDetailsById = () => {
    return `SELECT id, email,phone, status,role_id,profile_id FROM ${table_name} where  id = ?   `;
}

let getPendingUserById = () => {
    return `SELECT id, email,phone,status,role_id,profile_id FROM ${table_name} where  id = ?  and status = 2 `;
}
