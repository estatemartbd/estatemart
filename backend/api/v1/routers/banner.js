...
router.get('/list', [verifyToken, routeAccessChecker("bannerList")], async (req, res) => {
...
router.post('/list', [verifyToken, routeAccessChecker("bannerListLimit")], async (req, res) => {
...
router.post('/bannerFrontendList', [], async (req, res) => {
    ...
    if ([2, 3, 4].indexOf(reqData.banner_type) == -1) {
        return res.status(400).send({
            "success": false,
            "status": 400,
            "message": "Banner type should be  between 2 to 4."
        });
    }

    // let todayDate = new Date().toISOString().split('T')[0];
    let todayDate = await commonObject.getCustomDateTime(new Date());

    let result = await bannerModel.getBannerListByTypeAndDate(reqData.banner_type, todayDate);
...
router.post('/add', [verifyToken, routeAccessChecker("bannerAdd")], async (req, res) => {
    ...
    if ([4, '4'].includes(reqData.banner_type)) reqData.details = '#';
    else reqData.details = isEmpty(reqData.details) ? "#" : reqData.details;
...
router.put('/update', [verifyToken, routeAccessChecker("bannerUpdate")], async (req, res) => {
    ...
    if (existingDataById[0].banner_type !== reqData.banner_type) {
        ...
        if ([2, 3, 4].indexOf(reqData.banner_type) == -1) {
            ...
            willWeUpdate = 1;
            updateData.banner_type = reqData.banner_type;
        }
    }
...
router.delete('/delete', [verifyToken, routeAccessChecker("bannerDelete")], async (req, res) => {
...
router.put('/changeStatus', [verifyToken, routeAccessChecker("changeBannerStatus")], async (req, res) => {
