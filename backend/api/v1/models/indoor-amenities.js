...
const queries = require('../queries/indoor-amenities');
...
let getDetailsByIdAndWhereIn = async (id = []) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getDetailsByIdAndWhereIn(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
