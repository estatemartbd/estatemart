...
const queries = require('../queries/login-track');

let getLoggingTrackerById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getLoggingTrackerById(), [id], (error, result, fields) => {
...
let getLoggingTrackerByUserId = async (user_id = 0, time = "2022-01-23 07:29:56") => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getLoggingTrackerByUserId(), [user_id, time], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getLoggingTrackerByUserIdANDId = async (user_id = 0, id = 0) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getLoggingTrackerByUserIdANDId(), [user_id, id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getLoggingTrackerByUUID = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getLoggingTrackerByUUID(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let addNewLoggingTracker = async (info) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.addNewLoggingTracker(), [info], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let deleteLoggingTrackerDataByUUID = async (uuid = "") => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.deleteLoggingTrackerDataByUUID(), [uuid], (error, result, fields) => {
            if (error) reject(error)
            else {
                //console.log("as//");
                resolve(result)
