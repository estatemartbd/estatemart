...
let getRoleById = async (id = 0) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getRoleById(), [id], (error, result, fields) => {
...
let getRoleByIdentityId = async (identity_id = 0) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getRoleByIdentityId(), [identity_id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let getDetailsByIdAndWhereIn = async (workShopId = []) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getDetailsByIdAndWhereIn(), [workShopId], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
