...
let insertForgetPassword = async (info) => {
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.insertForgetPassword(), [info], (error, result, fields) => {
...
let getUserByToken = async (token = '') => { // get only active user
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.getUserByToken(), [token], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
...
let disableLinkById = async (id, conn) => {
    ...
    return new Promise((resolve, reject) => {
        connectionEstateMYSQL.query(queries.disableLinkById(), [id], (error, result, fields) => {
            if (error) reject(error)
            else resolve(result)
        });
    });
