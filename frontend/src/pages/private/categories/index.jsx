...
import { ServiceTable } from './categoriesTable'
...
const Service = () => {
  const [categoriesData, setServiceData] = useState()
    ...
    if (!id) {
      ...
      Swal.fire({
        ...
        if (value.isConfirmed === true) {
          categoriesService.deleteService(id).then(
            (res) => {
              if (res.data.success) {
                toast.success(res.data.message, {
                  theme: 'dark',
                })
                categoriesService.getServiceList().then((res) => {
                  setServiceData(res?.data?.data)
                })
              }
            },
  ...
  const handleChangeStatus = (id) => {
    if (!id) {
      ...
      Swal.fire({
        ...
        // console.log(value)
        if (value.isConfirmed === true) {
          categoriesService.changeStatus(id).then(
            (res) => {
              if (res.data.success) {
                toast.success(res.data.message, {
                  theme: 'dark',
      ...
      <div className="row">
        <div className="col-lg-4 col-xl-4 mb10">
          <div className="breadcrumb_content style2 mb30-991">
            <h2 className="breadcrumb_title">Categories List</h2>
            {/* <p>We are glad to see you again!</p> */}
          </div>
        </div>
        <div className="col-lg-8 col-xl-8">
          <div className="my_profile_setting_input text-right">
            {/* <Link to="/categories/add" className="btn btn2">
              Add Service
            </Link> */}
          </div>
        </div>
