...
const Package = () => {
  ...
  useEffect(() => {
    ...
    outdoorAmenitiesService.getList().then(
      (res) => {
    ...
    if (!id) {
      ...
      Swal.fire({
        ...
        if (value.isConfirmed === true) {
          outdoorAmenitiesService.outdoorAmenitiesDelete(id).then(
            (res) => {
              if (res.data.success) {
                toast.success(res.data.message, {
                  theme: 'dark',
                })
                outdoorAmenitiesService.getList().then((res) => {
                  setPackageData(res?.data?.data)
                })
              }
            },
  ...
  const handleChangeStatus = (id) => {
    if (!id) {
      ...
      Swal.fire({
        ...
        // console.log(value)
        if (value.isConfirmed === true) {
          outdoorAmenitiesService.changeStatus(id).then(
            (res) => {
              if (res.data.success) {
                toast.success(res.data.message, {
                  theme: 'dark',
      ...
      <div className="row">
        ...
        <div className="col-lg-8 col-xl-8">
          <div className="my_profile_setting_input text-right">
            <Link to="/auth/outdoor-amenities/add" className="btn btn2">
              Add New
            </Link>
          </div>
        </div>
